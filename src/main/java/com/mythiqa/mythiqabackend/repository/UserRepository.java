...
public interface UserRepository extends JpaRepository<User, String> {
    ...
    @Query("SELECT new com.mythiqa.mythiqabackend.dto.response.UserProfileDTO(u.username, u.description, u.profileBackgroundImgUrl, u.userProfileUrl, u.createdAt) FROM User u WHERE u.username = :username")
    Optional<UserProfileDTO> findUserProfileByUsername(@Param("username") String username);

    @Query("SELECT new com.mythiqa.mythiqabackend.dto.response.UserProfileImgDTO(u.userProfileUrl) FROM User u WHERE u.username = :username")
